...
#file settings
metadataOnly = False
basedir = 'J:\\Workgroups\\ARCH\\EDNA\\projecten_wansleeben\\projecten\\Vlaanderen\\notas'
...
#REST settings
providerName = 'VIOE_notities' 
vioe_url = 'https://loket.onroerenderfgoed.be/archeologie/notas/notas/'
headers = {'Accept': 'application/json', 
           'User-Agent': 'python-requests/2.18.1',
           'From': 'm.wansleeben@arch.leidenuniv.nl (EXALT)'  
          }
# https://loket.onroerenderfgoed.be/archeologie/notas/notas?publiek=False&sort=id&per_pagina=50&pagina=2
params = {'publiek': False,
          'sort': 'id',
          'per_pagina': 50,
          'pagina': 321 }   # deelresultaten van 50 stuks (= max)
...
csv_columns = ('provider','id','uri','naam','uitvoerder','melden_datum','besluit_datum','maatregelen','rapporttype','gemeenten','XYlocatie','projectcodes','relaties')
...
if response.status_code == requests.codes.ok:
 if response:
    ...
    pag = 2
    # Fetch the next 19 pages of 50 results
    while 'next' in response.links and pag <= 20:
    ...
    for project in data:
        ...
        doc['melden_datum'] = project['datum_melden'][:10]
        project_id = format(project['id'], '05d')
                              
        # Make a request for project data and store the response
        try:
          project_response = requests.get(project['uri'], headers=headers)
        ...
        if project_response.status_code == requests.codes.ok:
          ...
          doc['besluit_datum'] = project_data['datum_registreren_beslissing'][:19]
##        doc['updated'] = project_data['systemfields']['updated_at'][:19]    # niet in metadata notas
##        doc['projecttype'] = project_data['referentietype']['naam']         # niet in metadata notas
          doc['rapporttype'] = project_data['notatype']
##        doc['provincies'] = project_data['locatie']['provincies']           # niet in metadata notas
          doc['gemeenten'] = project_data['locatie']['gemeenten']

          # first coordinates
          point = project_data['locatie']['contour']['coordinates'][0][0][0]
          doc['XYlocatie'] = '(' + str(point[0]) + ',' + str(point[1]) + ')'

          doc['maatregelen'] = list()
          maatregelen = []
          for maatregel in project_data['maatregelen']:
              if maatregel['naam']:
                  maatregelen.append(maatregel['naam'])
          doc['maatregelen'] = list(set(maatregelen))   # remove duplicates

          doc['projectcodes'] = list()
          projectcodes = []
          for code in project_data['projectcodes']:
              if code['id']:
                  projectcodes.append(code['id'])
          doc['projectcodes'] = list(set(projectcodes))   # remove duplicates

          doc['relaties'] = list()
          relaties = []
          for relatie in project_data['relaties']:
              if relatie['uri']:
          ...
          for bijlage in bijlage_lijst:
            ...
            if bijlage['bijlagetype']['naam'] == 'Verslag van resultaten':  
              ...
              docurl = bijlage['url'] 
              bijlage_id = format(bijlage['id'], '06d')   # 05d
              file_naam = bijlage['bestandsnaam'].replace(':','-')      # : in de bestandsnaam vervangen door -
              ...
              if not metadataOnly:
                ...
                if r.status_code == requests.codes.ok:
                  ...
                  if r.headers['Content-Type'].lower() == 'application/pdf': 
                    # or  r.headers['Content-Type'].lower() == 'application/zip' or r.headers['Content-Type'].lower() == 'application/x-zip-compressed':
                    outfilenameStr = docsdir + '\\VIOEnota_' + project_id + '_' + bijlage_id + '_' + file_naam
                    if len(outfilenameStr) > 254:
                      outfilenameStr = outfilenameStr[0:249] + outfilenameStr[-4:]
                    print (format(i, '03d') + ' ' + outfilenameStr)
                    ...
                    print ('No pdf: '+ docurl)
                    errf.write(str(doc['id']) + ';' + str(bijlage['id']) + ';"' + file_naam + '";"";""; "No pdf"' + '\n')

                else:
                  print ('Error: '+ docurl)
                  errf.write(str(doc['id']) + ';' + str(bijlage['id']) + ';"' + file_naam + '";"";""; "No access"' + '\n')       
